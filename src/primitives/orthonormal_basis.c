...
The resulting basis uses:
  ...
  right   → perpendicular to up, computed from a temporary vector, r = (rx, uy,
		uz)
...
/*
Converts a vector v from standard basis (world space) into given orthonormal
basis (local). vl = B^(-1)v, where B^(-1) is the inverse matrix of basis
transformation matrix, v world vector vl = B^(T)v, the inverse matrix B^(-1) is
same as the transpose of B since as basis vectors are orthonormals
*/
...
vl = B^(-1)(v - O) --> vl = B^(T)(v - O),
	where B^(T) is the transpose matrix of the basis
...
/*
Converts a point from local to world, v = O
	+ Bvl where B is basis transformation batrix
and vl local point,
	B basis transformation matrix and O origin of basis in world cordinates
*/
t_vec3d	point_from_basis(t_vec3d vec, t_basis3d basis, t_vec3d origin)
{
	...
	p = origin;
	p.x = origin.x + vec.x * basis.right.x + vec.y * basis.up.x + vec.z
		* basis.forward.x;
	p.y = origin.y + vec.x * basis.right.y + vec.y * basis.up.y + vec.z
		* basis.forward.y;
	p.z = origin.z + vec.x * basis.right.z + vec.y * basis.up.z + vec.z
		* basis.forward.z;
	return (p);
}
