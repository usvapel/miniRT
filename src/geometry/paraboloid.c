...
bool paraboloid_hit(t_paraboloid *para, t_ray ray, t_hit *hit)
{
    ...
    t_ray r = local_ray(ray, p, para->base.pos); // ray in new coords
    float t[2];
	float disc;
	t_vec3d n_hit_pos;

	disc = solve_paraboloid_hit(r, new_paraboloid(new_vec3d(0, 0, 0), new_vec3d(0, 1, 0), para->focal), &t[0], &t[1]);
	if (disc < 0.0f || (t[0] < 0.0f && t[1] < 0.0f))
		return false;
    if (!valid_params(para->h, r, t[0], t[1]))
        return false;
    t[0] = get_valid_param(para->h, r, t[0], t[1]);
    n_hit_pos = get_point_on_ray(r, t[0]); // hit in object coords
    n_hit_pos = point_from_basis(n_hit_pos, p, para->base.pos); // convert from object to world point
...
float solve_paraboloid_hit(t_ray r, t_paraboloid para, float *t0, float *t1)
{
    ...
    b = 2 * (r.origin.z * r.udir.z + r.origin.x * r.udir.x) - 4 * para.focal * r.udir.y;
    c = (r.origin.z * r.origin.z + r.origin.x * r.origin.x) - 4 * para.focal * r.origin.y;
    disc = (b * b) - (4.0f * a * c);
    if (disc >= 0.0f)
    {
        sqrt_disc = sqrt(disc);
