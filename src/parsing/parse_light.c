...
static void	set_values(t_generic_light *g_light, t_point_light *light,
		t_vector *v, char **split)
{
	...
	g_light->base.material.reflect = 0.0f;
	g_light->base.material.refract = -1.0f;
...
void	init_light(t_vector *g_lights, char **split)
{
	...
	set_values(g_light, &g_light->point_light, v, split);
	free_split_vector(v);
