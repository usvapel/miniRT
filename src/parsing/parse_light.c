...
static void	set_values2(t_generic_light *g_light, t_vector *v, char **split)
{
	add_elem(v, safe_split(v, split[1]));
	add_elem(v, safe_split(v, split[2]));
	add_elem(v, safe_split(v, split[3]));
	...
	g_light->base.pos = parse_vec3d(v, v->data[0]);
	g_light->spot_light.axis = parse_vec3d(v, v->data[1]);
	g_light->base.axis = &g_light->spot_light.axis;
	g_light->brightness = ft_atof(((char ***)v->data)[2][0]);
	g_light->spot_light.range = ft_atof(((char ***)v->data)[3][0]);
	g_light->spot_light.fov = ft_atof(((char ***)v->data)[4][0]);
