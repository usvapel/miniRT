...
float					solve_sphere_hit(t_ray ray, t_sphere sphere, float *t0,
							float *t1);
...
bool					circle_hit(t_circle circ, t_ray ray, float *t0,
							float *t1);

// paraboloid
bool					paraboloid_hit(t_paraboloid *para, t_ray ray,
							t_hit *hit);
...
float					compute_axis_tbounds(t_cube cube, t_ray ray, int index,
							bool max);
void					set_min_max_tbounds(float *t1, float *t2, float *tmin,
							float *tmax);
t_cube					new_cube(float h, float w, float d);

// light
bool					light_hit(t_generic_light *light, t_ray ray,
							t_hit *hit);
bool					spot_light_hit(t_generic_light *spot, t_hit *hit,
							t_phong *phong);
t_vec3d					adjusted_light_pos(t_generic_light light);
t_object				*get_base_light(t_generic_light *light);

t_vec3d					get_shadow_attenuation(t_phong *p, t_hit hit,
